...
void tratarPlayer(char archivo[15],char ar[maxx+1][maxy+1],char ar2[maxx+1][maxy+1],char nombre_p1[40],char nombre_p2[40]); //carga barcos
void validarPlayer(FILE *fp, char caracteres[85],int *i,char ar[maxx+1][maxy+1], char nombre_p[40]); //carga barcos
void tratarCoordenadas(char **sub_ptr,int **i, int *iii, int *long_barco, int *len_coord,char ar[maxx+1][maxy+1],char direccion, int *aux_coordX, int *aux_coordY);
void verif_bm(char minas[maxx+1][maxy+1],char barcos[maxx+1][maxy+1],char tablero[maxx+1][maxy+1],int *cantb,int *gano,int jug);
void explot_carga(char minas[maxx+1][maxy+1],char barcos[maxx+1][maxy+1],char tablero[maxx+1][maxy+1],int *cantb,int *gano,int jug,int X,int Y);
...
void jugar(char minas2[maxx+1][maxy+1],char barcosj2[maxx+1][maxy+1],char tablero1[maxx+1][maxy+1],int *cantb,int *gano,int jug);
void Jugar_coordenadas(int *X,int *Y,char tablero[maxx+1][maxy+1]); //jugas las coordenadas
void proceso(char minas2[maxx+1][maxy+1],char barcosj2[maxx+1][maxy+1],char tablero1[maxx+1][maxy+1],int *cantb,int *gano,int jug,int X,int Y);
char barco_hundido(char barcosj2[maxx+1][maxy+1],int X,int Y,int tambarco);//se fija si el barco esta undido
void jugar_cpu(char minas2[maxx+1][maxy+1],char barcosj2[maxx+1][maxy+1],char tablero1[maxx+1][maxy+1],int *cantb,int *gano,int jug);
...
{
char minas1[maxx+1][maxy+1];
char minas2[maxx+1][maxy+1];
char barcosj1[maxx+1][maxy+1];
char barcosj2[maxx+1][maxy+1];
...
inicializar(minas1);
inicializar(minas2);
inicializar(barcosj1);//aca cargan los barcos
inicializar(barcosj2);//aca cargan los barcos
...
loading_minas(4);
cargar_minas(argv[2],minas1);	// en minas 1 secarga minas2.txt
cargar_minas(argv[1],minas2);	// en minas 2 secarga minas1.txt

loading_players(8);
tratarPlayer(argv[3],barcosj1,barcosj2,name1,name2);

verif_bm(minas1,barcosj1,tablero2,&cantb1,&gano,1);
verif_bm(minas2,barcosj2,tablero1,&cantb2,&gano,2);
	...
	case 1:     //CPU MODE
	{
		...
		while(gano==0){
			if(juega==1){
				...
				Enter();
				clrscr();
				mostrar_tableros(barcosj1,tablero1,name1);
				jugar(minas2,barcosj2,tablero1,&cantb2,&gano,juega);
				mostrar_tableros(barcosj1,tablero1,name1);
				juega=2;
				clrscr();
			}else{
				...
				Enter();
				clrscr();
				jugar_cpu(minas1,barcosj1,tablero2,&cantb1,&gano,juega);
				juega=1;
				clrscr();
				}else{
	...
	case 2:    //2 PLAYERS MODE
	{
		...
		while(gano==0){
			if(juega==1){
				...
				Enter();
				clrscr();
				mostrar_tableros(barcosj2,tablero2,name2);
				jugar(minas1,barcosj1,tablero2,&cantb1,&gano,juega);
            mostrar_tableros(barcosj2,tablero2,name2);
				juega=1;
				clrscr();
				}else{
...
//Pre{Que exista el archivo e inicializar el tablero}//
void tratarPlayer(char archivo[15],char ar[maxx+1][maxy+1],char ar2[maxx+1][maxy+1],char nombre_p1[40],char nombre_p2[40]){
	FILE *fp;
	char caracteres[85];
	int i = 0;
	int h,j;
	...
	validarPlayer(fp, caracteres, &i, ar, nombre_p1);
	validarPlayer(fp, caracteres, &i, ar2, nombre_p2);
...
void validarPlayer(FILE *fp, char caracteres[85],int *i,char ar[maxx+1][maxy+1], char nombre_p[40]){
	char aux_caracteres[85];
	int len;
	int len_ptr;
	int len_coord;
	...
	*i=*i+1;
			...
			if(( ptr=strtok( NULL, "-" )) != NULL){
				...
				if(( ptr=strtok( NULL, "-" )) != NULL){
					...
					if((long_barco >= iii) && ((sub_ptr=strtok( ptr, ";" )) != NULL)){
						tratarCoordenadas(&sub_ptr,&i,&iii,&long_barco,&len_coord,ar,direccion,&aux_coordX,&aux_coordY);
						iii++;
					}else{
	...
	if(fgets( caracteres, 85, fp) != NULL){
		if((strchr(caracteres,'"') != NULL)){
			if(*i >= 12){
				printf("Error: el maximo de jugadores es 2");
			}
		}
	}

		if(!(feof(fp)) && (strchr(caracteres,'"') == NULL)){
			printf("Error: hay mas de %d barcos", cantidadbarcos);
			exit(0);
		}else{
				if(*i <= cantidadbarcos){
					printf("Error: hay menos de 5 barcos", cantidadbarcos);
					exit(0);
				}
		}
...
for(i=1;i<=maxx;i++){
	for(j=1;j<=maxx;j++){
		if(minas[i][j]=='*' && (barcos[i][j]=='2' || barcos[i][j]=='3' || barcos[i][j]=='4' || barcos[i][j]=='5')){
			explot_carga(minas,barcos,tablero,&*cantb,&*gano,jug,i,j);
		}
	}
}
clrscr();
}
//Pos{Recorre los tableros y chequea cual debe explotar}//
/*-------------------------------------------------------------------------------------------------*/
...
int tambarco;
	tambarco=barcos[X][Y]-'0'; // atoi es solo para string
	barcos[X][Y]='X';
	minas[X][Y]='O';
	tablero[X][Y]='X';
...
printf("%d,%d\n",X,Y);
proceso(minas2,barcosj2,tablero1,&*cantb,&*gano,jug,X,Y);
}
...
do{
	...
	clrscr();
	if(*X<1 || *X>maxx){
		printf("Error en la coordenada X, ingese las cordenadas nuevamente\n");
	}
	if(*Y<1 || *Y>maxy){
		printf("Error en la coordenada Y, ingese las cordenadas nuevamente\n");
	}
	...
	for(i=X-movminas;i<=X+movminas;i++){
		for(j=Y-movminas;j<=Y+movminas;j++){
			if(i>0 && i<=maxx && j>0 && j<=maxy){
				proceso(minas2,barcosj2,tablero1,&*cantb,&*gano,jug,i,j);
			}
		}
	}
}else{
	if(barcosj2[X][Y]!='O' && barcosj2[X][Y]!='X'){
		if(barcosj2[X][Y]=='2' || barcosj2[X][Y]=='3' || barcosj2[X][Y]=='4' || barcosj2[X][Y]=='5'){
			printf("Barco tocado\n");
			tambarco=barcosj2[X][Y]-'0'; // atoi es solo para string
			barcosj2[X][Y]='X';
			tablero1[X][Y]='X';
			Enter();
			if(barco_hundido(barcosj2,X,Y,tambarco)=='V'){
				printf("Barco hundido\n");
				*cantb=*cantb-1;
				Enter();
				if(*cantb==0){
...
//Pre{...}//
void Ganador(int gano,int jug,char name1[85],char name2[85])//Poner los nombres del archivo jugador
{
if(gano==1){
